...
import { getLostItems, updateLostItem, deleteLostItem } from "../../api/adminService.js";

export default function LostItemsManagement() {
    ...
    // ⭐ EFFECT: Manages Auto-dismiss and continuous animation ⭐
    useEffect(() => {
        if (successMessage) {
    ...
    const fetchLostItems = async () => {
        ...
        try {
            ...
            setError("Failed to load lost items. Please check server connection.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const deleteItem = async (id) => {
        try {
            ...
            // ⭐ SIMPLIFIED SUCCESS MESSAGE FOR DELETION ⭐
    ...
    const editItem = async (editedItem) => {
        try {
            await updateLostItem(editedItem._id, editedItem);
            setItems((prev) =>
                prev.map((item) => (item._id === editedItem._id ? editedItem : item))
            );
            setError(null);
            
             // ⭐ SIMPLIFIED SUCCESS MESSAGE FOR UPDATING ⭐
    ...
    const sortedAndFilteredItems = useMemo(() => {
        ...
        let sortableItems = items.filter(
            (item) =>
    ...
    const handleDeleteConfirm = () => {
        if (itemToDelete) deleteItem(itemToDelete._id);
    ...
    if (loading) {
        return (
            <div className="container py-5 text-center">
                ...
                <p className="mt-2 text-muted">Loading lost items...</p>
            </div>
        );
    }
        ...
        <div className="container py-4">
            ...
            <style jsx global>{`
                ...
                .custom-toast-container {
                    ...
                    max-width: 300px; /* Keep it narrow */
                }
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h3 className="fw-semibold">Lost Items Management</h3>
                        ...
                        <table className="table table-striped table-hover align-middle mb-0">
                            <thead className="table-secondary">
                                <tr>
                                    ...
                                    >
                                        Location{" "}
                                        <FontAwesomeIcon icon={getSortIcon("location")} className="ms-2 small" />
                                    </th>
                                    <th
                                        scope="col"
                                        onClick={() => requestSort("dateLost")}
                                        style={{ cursor: "pointer" }}
                                        className="py-3"
                                    >
                                        Lost Date{" "}
                                        <FontAwesomeIcon icon={getSortIcon("dateLost")} className="ms-2 small" />
                                    </th>
                            ...
                            <tbody>
                                {sortedAndFilteredItems.length > 0 ? (
                                    sortedAndFilteredItems.map((it) => (
                                        <tr key={it._id}>
                                            <th scope="row" className="text-muted ps-4 small">
                                                #{it._id.slice(-4)}
                                            </th>
                                            <td>
                                            ...
                                            <td>{it.location}</td>
                                            <td>
                                                {it.dateLost ? new Date(it.dateLost).toLocaleDateString() : "N/A"}
                                            </td>
                                            <td>
                                    ...
                                    <tr>
                                        <td colSpan="5" className="text-center text-secondary py-4">
                                            No lost items match the current search criteria.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <Modal show={showDeleteModal} onHide={handleDeleteCancel} dialogClassName="admin-delete-modal-sm">
                <style type="text/css">{`
                ...
                <Modal.Body className="py-3 px-3">
                    <h6 className="mb-2 text-danger fw-semibold">Permanently Delete Lost Item?</h6>
                    <p className="small mb-2">
                        You are about to delete the lost item:
                        <strong className="text-dark">
                            {" "}
                            "{itemToDelete?.title}" (ID: #{itemToDelete?._id?.slice(-4)})
                        </strong>
                        .
                        <br />
                    ...
                    <Alert variant="warning" className="small py-1 px-2 mt-2 mb-0">
                        <strong className="fw-bold">Security Warning:</strong> Ensure no potential matches are
                        pending before deleting.
                    </Alert>
                </Modal.Body>
                ...
                <Modal.Body>
                    {itemToEdit && (
                        <Form>
                            ...
                            <Form.Group className="mb-2" controlId="editDateLost">
                                <Form.Label className="small mb-1">Date Lost</Form.Label>
                                <Form.Control
                                    size="sm"
                                    type="date"
                                    name="dateLost"
                                    value={itemToEdit.dateLost}
                                    onChange={handleEditChange}
                                />
                            </Form.Group>
